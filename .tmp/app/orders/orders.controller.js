...
angular.module('bhcmartApp').controller('OrdersCtrl', ['$scope', 'Order', 'Auth', 'Modal', 'Request', '$mdDialog', function ($scope, Order, Auth, Modal, Request, $mdDialog) {
  ...
  $scope.cancelorder = function (ev, orderid, ordernum) {
    ...
    var confirm = $mdDialog.prompt().title('Do you want to cancel the Order ' + ordernum).textContent('please state a reason').placeholder('reason').ariaLabel('reason').initialValue('reason').targetEvent(ev).ok('cancel order').cancel('dismiss');
  ...
  $scope.returnProduct = function (ev, order, productid, productname) {
    ...
    // Appending dialog to document.body to cover sidenav in docs app
    var confirm = $mdDialog.prompt().title('Do you want to return the Product ' + productname).textContent('please state a reason').placeholder('reason').ariaLabel('reason').initialValue('reason').targetEvent(ev).ok('Return Product').cancel('dismiss');
