...
describe('Product+Catalog', function () {
  ...
  var createCatalog = function createCatalog(done) {
    Catalog.find({}).remove().then(function () {
  ...
  it('should create a product with book catetory', function (done) {
    Product.create({ title: 'Broccoli', price: 123, categories: [books._id] }).then(function () {
...
describe('Finding products by category', function () {
  ...
  var createProductsCatalog = function createProductsCatalog(callback) {
    Catalog.find({}).remove().then(function () {
  ...
  it('should find 3 electronic products', function (done) {
    Product.find({ 'categories': electronics }).populate('categories').exec().then(function (products) {
  ...
  it('should find one vehicle and one clothing products', function (done) {
    Product.find({ 'categories': { $in: [vehicle, clothing] } }).populate('categories').exec().then(function (products) {
  ...
  it('should find _all_ products', function (done) {
    Product.find({ 'categories': { $in: mainCatalog.children } }).populate('categories').exec().then(function (products) {
...
//# sourceMappingURL=catalog.model.spec.js.map
