...
describe('Catalog API:', function () {
  ...
  describe('GET /api/catalogs', function () {
    ...
    beforeEach(function (done) {
      request(app).get('/api/catalogs').expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        catalogs = res.body;
  ...
  describe('POST /api/catalogs', function () {
    beforeEach(function (done) {
      request(app).post('/api/catalogs').send({
        name: 'New Catalog'
  ...
  describe('GET /api/catalogs/:id', function () {
    ...
    beforeEach(function (done) {
      request(app).get('/api/catalogs/' + newCatalog._id).expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        catalog = res.body;
  ...
  describe('PUT /api/catalogs/:id', function () {
    ...
    beforeEach(function (done) {
      request(app).put('/api/catalogs/' + newCatalog._id).send({
  ...
  describe('DELETE /api/catalogs/:id', function () {
    ...
    it('should respond with 204 on successful removal', function (done) {
      request(app)['delete']('/api/catalogs/' + newCatalog._id).expect(204).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
    ...
    it('should respond with 404 when catalog does not exist', function (done) {
      request(app)['delete']('/api/catalogs/' + newCatalog._id).expect(404).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
...
//# sourceMappingURL=catalog.integration.js.map
