...
function productsInCategory(limit) {
  ...
  return function (catalog) {
    ...
    }).limit(limit).populate({ path: 'categories', select: 'name' }).populate({ path: 'reviews', select: 'rating' }).populate({ path: 'images', select: 'imageUrl' }).execAsync();
  };
}
...
exports.index = function (req, res) {
  Product.find().sort({ stock: 1 }).populate({ path: "categories", select: "name" }).execAsync().then(responseWithResult(res))['catch'](handleError(res));
};
...
exports.catalog = function (req, res) {
  ...
  Catalog.findOne({
    slug: req.params.slug
  }).execAsync().then(productsInCategory(limit)).then(responseWithResult(res))['catch'](handleError(res));
};
  ...
  return function (entity) {
    if (entity) {
      var variants = _.map(entity.variants, function (variantId) {
        return Variant.findByIdAsync(variantId).then(handleEntityNotFound(res));
      });
      return _Promise.all(variants);
    } else {
      return null;
    }
  };
...
exports.createVariant = function (req, res) {
  Variant.createAsync(req.body).then(linkVariantProduct(res, req.params.id)).then(responseWithResult(res, 201))['catch'](handleError(res));
};

// Gets variants for a single Product from the DB
exports.indexVariant = function (req, res) {
  Product.findByIdAsync(req.params.id).then(handleEntityNotFound(res)).then(getVariants(res)).then(responseWithResult(res))['catch'](handleError(res));
};
...
exports.updateVariant = function (req, res) {
  ...
  Variant.findByIdAsync(req.params.variant_id).then(handleEntityNotFound(res)).then(saveUpdates(req.body)).then(responseWithResult(res))['catch'](handleError(res));
};

// Deletes a variant from the DB
exports.destroyVariant = function (req, res) {

  Variant.findByIdAsync(req.params.id).then(handleEntityNotFound(res)).then(function (entity) {
    var variant = entity;
    return Product.findByIdAsync(req.params.id).then(handleEntityNotFound(res)).then(removeVariantId(req.params.variant_id, variant));
